        ...
        async function fetchData() {
            try {
                ...
                for (let i = 0; i < clinicsArray.length; i++) {
                    const diagnosesSnapshot = await getDocs(collection(db, clinicsArray[i], "patients", "patientlist", user.uid, "diagnoses"));
        ...
        <div className='relative w-full lato'>
            ...
            <div className='bg-slate-100 rounded-b-lg'>
                {diagnoses.map((diagnosis, index) => (
                    <div className='grid grid-cols-5 text-center p-4 border border-black'>
                        ...
                        <button onClick={() => showForm(index)} className='hover:bg-slate-200 rounded-lg border border-black enriqueta'>View Form</button>
                    </div>
                ))}
